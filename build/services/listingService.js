...
const mergeValue = (record, key, value) => {
...
const extractSuumoRent = ($) => cleanupValue($(".property_view_main-emphasis").first().text());
...
const assignDepositAndKeyMoney = (output, combinedValue, shikikinValue, reikinValue) => {
    if (combinedValue) {
...
const extractCostItem = (value, label) => {
    if (!value) {
        return null;
    }
...
export const scrapeListing = async (input) => {
